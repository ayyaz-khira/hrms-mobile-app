...
export default function HodApprovalsScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      const loadHodApprovals = async () => {
        const userId = await AsyncStorage.getItem('user_id');
        if (userId) {
          await fetchHodApprovals(userId);
        }
      };
      loadHodApprovals();
    }, [])
  ...
  const handleApprove = async (request: LeaveRequest) => {
    Alert.alert(
      'HOD Approve Leave',
      `Are you sure you want to approve this HOD request for ${request.employee_name || request.employee}?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            const success = await approveLeaveRequest(request.name, 'HOD');
            setProcessingId(null);
            if (success) {
              setModalVisible(false);
              Alert.alert('Success', 'HOD leave request approved successfully.');
            } else {
              Alert.alert('Error', 'Failed to approve HOD leave request.');
            }
          },
        },
      ]
  ...
  const handleReject = async (request: LeaveRequest) => {
    Alert.alert(
      'HOD Reject Leave',
      `Are you sure you want to reject this HOD request for ${request.employee_name || request.employee}?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Reject',
          ...
          onPress: async () => {
            setProcessingId(request.name);
            const success = await rejectLeaveRequest(request.name, 'HOD');
            setProcessingId(null);
            if (success) {
              setModalVisible(false);
              Alert.alert('Success', 'HOD leave request rejected.');
            } else {
              Alert.alert('Error', 'Failed to reject HOD leave request.');
            }
          },
        },
      ]
  ...
  return (
    <View style={[styles.mainContainer, { backgroundColor: C.bg }]}>
        ...
        <View style={[styles.headerBg, { backgroundColor: C.dark }]}>
          <SafeAreaView edges={['top']}>
            <View style={styles.headerRow}>
              <View style={{ width: 40 }} />
              <Text style={styles.headerTitle}>HOD Approvals</Text>
              <View style={{ width: 40 }} />
            </View>
          </SafeAreaView>
        </View>
      ...
      >
        {loading && hodApprovals.length === 0 ? (
          <View style={styles.centerContainer}>
            <ActivityIndicator size="large" color={C.primary} />
          </View>
        ) : hodApprovals.length === 0 ? (
          <View style={[styles.emptyCard, { backgroundColor: C.card }]}>
            <IconSymbol name="calendar.badge.exclamationmark" size={48} color={C.gray100} />
            <Text style={[styles.emptyText, { color: C.text }]}>No HOD Approvals Pending</Text>
            <Text style={[styles.emptySub, { color: C.subText }]}>No HOD leave requests need action currently.</Text>
          </View>
        ) : (
            ...
            {hodApprovals.map((item) => (
              ...
              >
                <View style={styles.requestTop}>
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.empName, { color: C.text }]}>{item.employee_name || item.employee}</Text>
                    <Text style={[styles.leaveType, { color: C.primary }]}>{item.leave_type}</Text>
                    <Text style={[styles.dateRange, { color: C.subText }]}>
                      {item.from_date} to {item.to_date}
                    </Text>
                  </View>
                  <View style={[styles.badge, { backgroundColor: C.primary + '15' }]}>
                    <Text style={[styles.badgeText, { color: C.primary }]}>{item.total_leave_days} Days</Text>
                  </View>
                </View>
          ...
          <View style={styles.modalOverlay}>
            ...
            <View style={[styles.modalCard, { backgroundColor: C.card }]}>
              <View style={styles.modalHeader}>
                <Text style={[styles.modalTitle, { color: C.text }]}>HOD Leave Details</Text>
